...
import { Tunemanager } from '../main/tunemanager.js';
import { ABCplayer } from 'src/services/abcplayer.js';
...
export class Tunemanagersearch extends Component {
  ...
  generatehtml() {
    ...
    bg-opacity-75 flex items-center justify-center">
            ...
            <i class="fa fa-times-circle fa-2x"></i></p>
                ...
                Añadir temas desde thesessio</h2>
                <input id="managertunesearch" type="text" 
                placeholder="escriba parte del nombre">
                <p class="info mt-6 mb-2"></p>
                <ul class="results bg-slate-300 text-slate-50 p-2 
                overflow-auto h-auto"></ul>
          </div>
        </div>`;
  }
  /**
      ...
      el.insertAdjacentHTML(
        ...
        `<div class="details"><p>Alias: 
          ${this.details.aliases.join(' / ')}</p>
                ...
                <button class="addtune bg-white text-slate-600
                uppercase p-2 font-bold mt-2 rounded-md">Añadir</button>
                </div>`
      );
  ...
  preparedata(event) {
    ...
    const alreadysaved = manager.tunes.some(
      (tune) =>
        tune?.References &&
    ...
    if (!alreadysaved) {
      ...
      parent.insertAdjacentHTML(
        'afterbegin',
        `<p class="text-white bg-red-400 p-1 mb-1 
          font-medium uppercase text-xs">
          Ya hay un tema con esta referencia.</p>`
      );
    }
  }
  ...
  /**
   * Save general tune reference in xano
   * @param {*} data
   * @param {*} el
   */
  async savetune(data, el) {
    try {
      ...
      if (result) {
        ...
        Controller.activeMenuComponent.rendertunes();
        el.remove();
      }
    } catch (error) {
      console.log(error);
