...
export class Tunemanageredit extends Component {
  ...
  generatehtml() {
    const sessionbool =
      this.data?.References && this.data.References[0]?.service_name
        ? this.data.References.find(
            (item) => item.service_name == 'thesession.org'
          )
        : false;
    const irishtunebool =
      this.data?.References && this.data.References[0]?.service_name
        ? this.data.References.find(
            (item) => item.service_name == 'irishtune.info'
          )
        : false;
    const tunearchbool =
      this.data?.References && this.data.References[0]?.service_name
        ? this.data.References.find(
            (item) => item.service_name == 'tunearch.org'
          )
        : false;
    const tonalities =
      this.data?.Modes_played && this.data.Modes_played[0]?.key
        ? this.data.Modes_played.map((mode) => mode.Key + ' ' + mode.Mode)
        : [];
    let sessionbutton = '';
    if (sessionbool) {
                ...
                <div class="owndata basis-1/2">
                    ...
                    <div class="flex gap-3">
                        <div>
                            ...
                            <p class="editthesession border" contenteditable="true">${
                              sessionbool ? sessionbool.service_ID : ''
                            }</p>
                        </div>
                        <div>
                            <label class="uppercase text-slate-400 text-sm mt-3">irishtune</label>
                            <p class="editirishtune border" contenteditable="true">${
                              irishtunebool ? irishtunebool.service_ID : ''
                            }</p>
                        </div>
                        <div>
                            <label class="uppercase text-slate-400 text-sm mt-3">tunearch</label>
                            <p class="edittunearch border" contenteditable="true">${
                              tunearchbool ? tunearchbool.service_ID : ''
                            }</p>
                        </div>
                    </div>

                    <label class="uppercase text-slate-400 text-sm mt-3">titulo</label>
                    <p class="editmainname border" contenteditable="true">${
                      this.data.main_name
                    }</p>
                    <label class="uppercase text-slate-400 text-sm mt-3">titulo para orden</label>
                    <p class="editsortname border" contenteditable="true">${
                      this.data.sortname.length > 0
                        ? this.data.sortname
                        : 'sin alias'
                    }</p>

                    <label class="uppercase text-slate-400 text-sm mt-3">aliases</label>
                    <p class="editalias border" contenteditable="true">${
                      this.data?.other_names
                        ? this.data.other_names.join(' / ')
                        : ''
                    }</p>
  ...
  generatesessionhtml(tune) {
    ...
    const recordings = tune.recordings.map(
      (element) =>
        `<li><strong>${element.artist.name}</strong> - ${element.name}</li>`
    );
    const videolinks = tune.comments.map((comment) => comment.content);
    const comments = videolinks.map(
      (item) => `<li class="border-b mb-2">${item}</li>`
    );
    this.videolinks = videolinks.join('').match(youtubesearch);
    const settings = tune.settings.map(
      (
        setting
      ) => `<li class="py-2"><span class="addtoform"><i class="fa fa-plus-circle fa-lg"></i></span> 
    ...
    return `
      ...
      <details>
      <summary class="uppercase text-slate-400 text-sm mt-1">Settings ABC ${
        settings.length
      }</summary>
        ...
        <details>
        <summary class="uppercase text-slate-400 text-sm mt-1">Grabaciones ${
          recordings.length
        }</summary>
        ...
        <details>
        <summary class="uppercase text-slate-400 text-sm mt-1">Comentarios ${
          comments.length
        }</summary>
        <ul>${comments.join('')}</ul>
        </details>
  ...
  setup() {
    ...
    this.element
      .querySelector('.updatetune')
      .addEventListener('click', this.updatetune.bind(this));

    if (this.element?.querySelector('.refreshThesession')) {
      this.element
        .querySelector('.refreshThesession')
        .addEventListener('click', this.refreshsession.bind(this));
    }
  }
  ...
  savefromsessiontoform(event) {
    ...
    if (el.dataset.target) {
      const mytarget = this.element.querySelector(
        '.owndata .' + el.dataset.target
      );
  ...
  checkservicesid(array) {
    ...
    array.forEach((service) => {
      const identifier = service.split('.');
      const value = this.element.querySelector(
        '#modaledittunemanager .edit' + identifier[0]
      ).innerText;
      if (value.length > 0) {
        References.push({
  ...
  async updatetune(event) {
    ...
    const References = this.checkservicesid([
      'thesession.org',
      'irishtune.info',
      'tunearch.org',
    ]);
    const Modes_played = Utils.converttones(
      this.element
        .querySelector('#modaledittunemanager .editmodesplayed')
        .innerText.split('/')
    );
    const params = {
      main_name: this.element.querySelector(
        '#modaledittunemanager .editmainname'
      ).innerText,
      other_names: this.element
        .querySelector('#modaledittunemanager .editalias')
        .innerText.split('/'),
      type: this.element.querySelector('#modaledittunemanager .edittype')
        .innerText,
      author: this.element.querySelector('#modaledittunemanager .editautor')
        .innerText,
      time: this.element.querySelector('#modaledittunemanager .edittime')
        .innerText,
      tradition: this.element
        .querySelector('#modaledittunemanager .edittradition')
        .innerText.split('/'),
      References: References,
      Modes_played: Modes_played,
      Estructure: this.element.querySelector(
        '#modaledittunemanager .editestructure'
      ).innerText,
      compasses: this.element.querySelector(
        '#modaledittunemanager .editcompases'
      ).innerText,
      trivia: this.element.querySelector('#modaledittunemanager .edittrivia')
        .innerText,
      popularity: this.element.querySelector(
        '#modaledittunemanager .editpopularity'
      ).innerText,
      sortname: this.element.querySelector(
        '#modaledittunemanager .editsortname'
      ).innerText,
      ABCsample: this.element.querySelector(
        '#modaledittunemanager .editabcsample'
      ).innerText,
    };
    try {
      ...
      if (result) {
        ...
        const manager = Controller.activeMenuComponent;
        manager.rendertunes();
        this.remove();
      }
    } catch (error) {
