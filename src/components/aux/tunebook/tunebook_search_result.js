...
export class Tunesearchresult extends Component {
  ...
  generatehtml() {
    return `
    <li class="${
      this.isintunebook ? 'opacity-50' : 'cursor-pointer hover:bg-white/25'
    }
    ...
    data-id="${this.tunedetail.id}">
        ...
        <span class="text-xs uppercase">${
          this.tunedetail?.tradition
            ? this.tunedetail.tradition.join(' · ')
            : ''
        }</span>
        ${
          this.tunedetail.popularity
            ? `<span class="ml-auto bg-slate-600
   ...
   */
  showmodaltune() {
    const tunebook = Controller.activeMenuComponent;
    tunebook.subelement = new Tuneaddtobook(
      'addtunetobook',
      tunebook.htmlelement,
      this.tunedetail.id
    );
  }
}
