...
export class Tunesearch extends Component {
  ...
  search(event) {
    ...
    if (string.length > 0) {
      ...
      const result = this.tunes.filter((tune) =>
        tune.other_names.some((name) => name.toLowerCase().includes(string))
      );
  ...
  async suggestion(event) {
    ...
    if (suggestion) {
      ...
      if (result) {
        this.setup();
        new MyNotification('success', `Se ha guardado su petición.`);
      } else {
        this.data = backup;
