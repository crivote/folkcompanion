...
export class Tuneaddtobook extends Component {
  ...
  async setup() {
    this.pics = await apis.Pexels.search(
      this.isNew ? this.tune.other_names[0] : this.tune.prefered_name
    );
  ...
  generatehtml() {
    ...
    const statuslabel = this.isNew
      ? Data.status[0].label
      : Utils.getstatus(this.tune.status_num).label;

    const mytags =
      this.isNew || this.tune.tags.length == 0 ? '' : this.tune.tags.join(' ');

    const mynotes =
      this.isNew || this.tune.notes.trim() == '' ? '' : this.tune.notes;
