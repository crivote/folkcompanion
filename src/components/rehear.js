...
export class Rehear extends Component {
  ...
  assignPointsTunes() {
    ...
    const pointsArray = filteredTunes.map((tune) => {
      ...
      const factor = Data.status.find(
        (status) => status.value == tune.status_num
      );
   ...
   */
  generatehtml() {
    const filters = `<select class="tunesorting text-sm bg-cyan-200 text-cyan-500 p-1 
          rounded-md border-0">
          ${this.criterialist
            .map(
  ...
  findSimilarTune(tune) {
    let foundTune = this.tunelist.findIndex(
      (item) =>
        item.tuneref.type === tune.tuneref.type &&
        item.prefered_tone === tune.prefered_tone
    );
    if (foundTune === -1) {
      foundTune = this.tunelist.findIndex(
        (item) =>
          item.tuneref.type === tune.tuneref.type &&
    ...
    if (foundTune === -1) {
      foundTune = this.tunelist.findIndex(
        (item) => item.tuneref.type === tune.tuneref.type
      );
    }
    if (foundTune !== -1) {
  ...
  renderSets() {
    ...
    this.tunesets.forEach((set) => {
      this.contentZone.insertAdjacentHTML(
        'beforeend',
        '<h5 class="mt-4">Set</h5>'
      );
      set.forEach((tune) => {
        this.tuneinstances.push(
          new RehearTune('tune' + tune.id, this.contentZone, tune)
        );
      });
    });
  }
