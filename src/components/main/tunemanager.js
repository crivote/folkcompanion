...
import { Tuneformanager } from '../aux/tunemanager_tune.js';
import { Tunemanagersearch } from '../aux/tunemanager_search.js';
...
export class Tunemanager extends Component {
   ...
   */
  filter() {
    const valtext = this.element
      .querySelector('.tunes_search')
      .value.toLowerCase();
    const valtype = this.element
      .querySelector('.typetune_search')
      .value.toLowerCase();
    const valtrad = this.element
      .querySelector('.origintune_search')
      .value.toLowerCase();

    this.filtered = Data.tunes.filter((tune) => {
      let val1 = true;
      if (valtext) {
        val1 = tune.other_names.join().toLowerCase().includes(valtext);
      }
      let val2 = true;
      if (valtype) {
