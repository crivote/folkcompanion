...
export class Tunebasicedit extends Component {
            ...
            <div class="bg-white p-8 rounded-xl shadow-lg w-1/2 lg:w-1/3 relative">
                ...
                <form id="loginform">
                    ...
                    ${Utils.generateformfield(
                      ...
                      'status de ejecución',
                      this.tune.status,
                      Data.status.map((status) => status.label)
                    )}
  ...
  addListeners() {
    ...
    this.element
      .querySelector('form')
      .addEventListener('submit', this.savetune.bind(this));
  ...
  async savetune(event) {
    ...
    const params = {
      ...
      rehearsal_days: this.element.querySelector('input[name="rehearsals"]')
        .value,
      last_rehearsals: Array.isArray(this.data.last_rehearsals)
        ? [
            this.element.querySelector('input[name="lastrehearsal"]').value,
    ...
    try {
      ...
      if (result) {
        ...
        let mitune = Controller.screens.Tunebook.tunebook.find(
